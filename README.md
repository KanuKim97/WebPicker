...
- `/요약`: summarize the active page.
- `/번역`: translate selected text or the active page.
- `/스크린샷`: capture the visible tab and ask the active model to analyze it.
- `/자동화`: propose one DOM action and execute it only after explicit approval.
- `/모델`: search and select an installed Ollama model.
- `/컨텍스트`: include or exclude the active page from regular chat.
- `/언어` or `/language`: switch the interface and AI response language.
- `/새대화`: clear the current conversation and task results.
- `/새로고침`: reload the WebPicker extension after confirmation.
